...
int prepare_create_hashtable (struct prepare *prepare)
{
	...
	snprintf(mys1.sql_buffer, MAX_SQL_BUFF, "SELECT `suppression_list_id` FROM `msg_to_suppression` WHERE `msg_id` = %d", prepare->msg_id);
	...
	if (count > 0)
	{
		snprintf(mys1.sql_buffer, MAX_SQL_BUFF, "SELECT COUNT(`email`) FROM `email_to_sup` WHERE %s", sql_w);
		...
		snprintf(mys1.sql_buffer, MAX_SQL_BUFF, "SELECT `email` FROM `email_to_sup` WHERE %s", sql_w);
...
int prepare_lists (struct prepare *prepare)
{
	...
	snprintf(mys1.sql_buffer, MAX_SQL_BUFF, "SELECT `user`.`username`, `user`.`id` FROM `msg_to_list`, `user` \
											 WHERE `msg_to_list`.`msg_id` = %d AND `user`.`id` = `msg_to_list`.`list_id`", prepare->msg_id);
...
int main (int argc, const char* argv[])
{
	...
	int 			i, j, k, l, sup_count, good_count, num_fields, extra, m, n, o;
	char 			tmp[3000], tmp_subject[320], *buffer, last_id[100];
	...
	prepare_create_hashtable(&prepare);
	...
	for (k=0;k<prepare.list_count;k++)
	{
		...
		{
			snprintf(mys1.sql_buffer, MAX_SQL_BUFF, "SELECT local, domain, CONCAT(local, '@', domain) as email, id %s FROM %s.%s;", prepare.sql_extra, prepare.lists[k].name, prepare.tables[i].name);
